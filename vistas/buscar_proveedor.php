...
<body>
    ...
    <form id="BPForm" class="rounded-3 form_search" action="buscar_proveedor.php" method="get">
        ...
        <input id="buspro" value="<?php echo $busqueda; ?>" type="text" name="busqueda" placeholder="Buscar proveedor" aria-label="Search" class="rounded-3">
        <button class="btn btn-success btn_search" type="submit" value="Buscar"><i class="fas fa-search"></i></button>
        <a href="proveedores.php"><button class="btn btn-warning" type="button"><i class="fas fa-book-open"></i></button></a>
    </form>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <?php
                                ...
                                //Consulta a la base de datos
                                $sql_registe= ("SELECT * FROM proveedores
                                    WHERE (id LIKE '%$busqueda%' OR
                                    nombrep LIKE '%$busqueda%' OR
                                    telefonop LIKE '%$busqueda%' OR
                                    direccionp LIKE '%$busqueda%' OR
                                    correop LIKE '%$busqueda%' OR
                                    rfcp LIKE '%$busqueda%' OR
                                    productosp LIKE '%$busqueda%' OR
                                    empresap LIKE '%busqueda%')");
